...
def get_market_skills(role: str, domain: str) -> dict:
    ...
    try:
        from market_research import get_market_trends
    except ImportError:
        def get_market_trends(q): return "Market research unavailable."
...
def explain_subtopic(topic: str, subtopic: str, domain: str,
    ...
    try:
        from rag_service import generate_rag_context
        from personal_rag_service import get_personal_context
    except ImportError:
        def generate_rag_context(*args): return ""
        def get_personal_context(*args): return ""
...
{{
  ...
  "d2_code": "Generate a professional, multi-container D2 diagram. \\nSyntax rules: \\n1. Use 'Connection -> Connection: Label' for arrows. \\n2. Group blocks in Container {{ ... }}. \\n3. ALWAYS provide a string label after the colon for connections. \\n4. NO trailing colons without values. \\n5. Example: Client.Browser -> Server: 'Fetch JSON'. \\n6. NO icons or markdown fences.",
...
"""
    ...
    try:
        ...
        # Extract and sanitize d2_code (ensure no markdown fences if AI accidentally included them)
        ...
        # Fetch visuals using Wikipedia
        image_url = get_wikipedia_image(data.get("visual_query") or subtopic)
...
{{
  ...
  "d2_code": "D2 architecture diagram. Rules: 1. Use Container {{ ... }} 2. Use 'A -> B: Label' 3. NO icons or markdown fences.",
...
Return ONLY valid JSON and include the D2 code for a visual architecture. Keep labels simple and ALWAYS after colons."""
    ...
    if d2_code and isinstance(d2_code, str):
        ...
        try:
            ...
            if res.ok:
                ...
                print(f"Teacher PDF diagram error (Kroki status {res.status_code}): {res.text[:100]}")
        except Exception as e:
            print(f"Teacher PDF diagram error: {e}")
